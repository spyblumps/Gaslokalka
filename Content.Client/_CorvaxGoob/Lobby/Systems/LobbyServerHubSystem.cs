...
using Content.Shared._CorvaxGoob.Lobby;
using Robust.Shared.Timing;

namespace Content.Client._CorvaxGoob.Lobby.Systems;
...
public sealed partial class LobbyServerHubSystem : EntitySystem
{
    ...
    private readonly Dictionary<string, LobbyServerHubStatus> _statuses = new();
    ...
    {
        return _statuses.TryGetValue(NormalizeAddress(address), out status!);
    }
    ...
    private void OnStatusResponse(LobbyServerHubStatusResponseEvent response)
    {
        ...
        foreach (var status in response.Statuses)
            _statuses[NormalizeAddress(status.Address)] = status;
